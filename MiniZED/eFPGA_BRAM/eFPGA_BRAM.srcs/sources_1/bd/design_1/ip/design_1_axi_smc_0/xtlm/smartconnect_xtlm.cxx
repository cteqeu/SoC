...
smartconnect_xtlm::smartconnect_xtlm(sc_core::sc_module_name name,
    ...
    smc_num_mi = stoi(smc_properties["NUM_MI"]);
        ...
        for (int i = 0; i < smc_num_mi; i++) {
         ...
         std::cout << "Pushing MI_burst_size: " << burst_size << "\n";
    ...
    for (int i = 0; i < smc_num_si; i++) {
         ...
         std::cout << "Initializing sockets and utils SXI: " << std::dec << i << "\n";
         saxi_rd_util.push_back(new xtlm::xtlm_aximm_target_rd_socket_util("saxi_rd_util_" + static_cast<char>(i), xtlm::aximm::TRANSACTION, 32));
         saxi_wr_util.push_back(new xtlm::xtlm_aximm_target_wr_socket_util("saxi_wr_util_" + static_cast<char>(i), xtlm::aximm::TRANSACTION, 32));
        ...
        std::cout << "Binding sockets and utils SXI: " << std::dec << i << "\n";
    ...
    for (int num_mi = 0; num_mi < smc_num_mi; num_mi++) {
        std::cout << "Initializing sockets and utils MXI: " << std::dec << num_mi << "\n";
        ...
        std::cout << "Binding sockets and utils MXI: " << std::dec << num_mi << "\n";
    ...
    std::cout << "Initializing SC_METHODS \n";
    ...
    dont_initialize();
    std::cout << "DONE Initializing SC_METHODS \n";
}
...
void smartconnect_xtlm::print_log(std::string port, int port_id, std::string rw, std::string action, xtlm::aximm_payload* &trans_ptr) {
   if (DEBUG == 1) {
      std::cout << std::setw(35) << std::left << std::string(this->name()) << ":";
      std::cout << std::setw(5) << std::left << port << std::setw(5) << std::left << std::dec << port_id;
      std::cout << std::setw(8) << std::left << rw;
      std::cout << std::setw(25) << std::left << action;
      std::cout << std::setw(8) << std::left << "TRANS:" << std::setw(10) << std::left << trans_ptr;
      std::cout << "\n";
   }
}
...
bool smartconnect_xtlm::is_address_hit(xtlm::aximm_payload* &trans_ptr, int num_si, unsigned long long &si_sep_route, int &m_burst_length, unsigned int &m_burst_size) {
   ...
   std::cout << "is_address_hit: SI_num_seg: " << std::dec << SI_num_seg << "\n";
...
//         sep_route                     = 0;
         ...
         std::cout << "is_address_hit(): SI " << num_si << " IS CASCADED: sep_route: " << std::hex << sep_route << "\n";
         std::cout << "is_address_hit(): SI " << num_si << " IS CASCADED: exten is: " << exten << "\n";
      } else {
         sep_route = SI_m_properties[num_si].getLongLong( seg_index + "SEP_ROUTE");
         slave     = ((1 << num_mi_log2) - 1) & sep_route;
         std::cout << "is_address_hit(): SI " << num_si << " NOT CASCADED: sep_route: 0x" << std::hex << sep_route << "\n";
      }

      std::cout << std::hex << "Address " << baseaddr << " : " << high_addr << "\n";
      ...
      if ( (baseaddr <= addr && addr < high_addr) || si_cascaded ) {
         ...
         if (command == xtlm::xtlm_command::XTLM_READ_COMMAND) {
            slave_rd_req = slave;
            std::cout << "SMARTCONNECT_XTLM: is_address_hit(): Address hit slave_rd_req: " << slave_rd_req << "\n";
            std::cout << "                   Base address: " << std::hex << baseaddr << "\n";
            std::cout << "                   High address: " << std::hex << high_addr - 1 << "\n";
            std::cout << "                   Slave       : " << slave << "\n";
            std::cout << "                   MI_burst_length[" << i << "]: " << MI_burst_length[slave] << "\n";
            std::cout << "                   MI_burst_size[" << i << "]: " << MI_burst_size[slave] << "\n\n";
         } else if (command == xtlm::xtlm_command::XTLM_WRITE_COMMAND) {
            slave_wr_req = slave;
            std::cout << "SMARTCONNECT_XTLM: is_address_hit(): Address hit slave_wr_req: " << slave_wr_req << "\n";
            std::cout << "                   Base address: " << std::hex << baseaddr << "\n";
            std::cout << "                   High address: " << std::hex << high_addr - 1 << "\n";
            std::cout << "                   Slave       : " << slave << "\n";
            std::cout << "                   MI_burst_length[" << i << "]: " << MI_burst_length[slave] << "\n";
         ...
         if (MI_m_properties[slave].getInt("IS_CASCADED") == 1) {
            ...
            std::cout << "is_address_hit(): MI " << slave << " IS CASCADED: sep_route is: " << std::bitset<64>(sep_route) << "\n";
            std::cout << "is_address_hit(): MI " << slave << " IS CASCADED: setting si_sep_route: " << std::bitset<64>(si_sep_route) << "\n";
         }
...
void smartconnect_xtlm::create_burst_transaction(std::list<xtlm::aximm_payload*> &saxi_vec, std::list<xtlm::aximm_payload*> &maxi_vec, 
   ...
   std::cout << "BEGIN Incoming Paylod information\n";
   trans_ptr->get_log(payload_msg, 3);
   std::cout << payload_msg << "\n";
   payload_msg = "";
   std::cout << "END Incoming Payload information\n\n";
...
//   int m_num_burst = decode_burst_size(m_burst_size);
   ...
   if(m_burst_size > s_burst_size) {
      ...
      s_data_size += (valid_byte & mask);
       std::cout << std::dec << "s_data_size buffer modified to:" << s_data_size << "\n\n";
   }
   ...
   if(!mi_cascaded) {
      if (trans_ptr->get_command() == xtlm::XTLM_WRITE_COMMAND) {
         ...
         std::cout << "Filled: map_wr_si_to_nummi[" << trans_ptr << "]: " << map_wr_si_to_nummi[trans_ptr] << "\n";
         map_wr_si_to_numsi[trans_ptr] = numsi;                //Maps si_trans to corresponding SI port
         std::cout << "Filled: map_wr_si_to_numsi[" << trans_ptr << "]: " << map_wr_si_to_numsi[trans_ptr] << "\n";
         ...
         std::cout << "Filled: map_rd_si_to_nummi[" << trans_ptr << "]: " << map_rd_si_to_nummi[trans_ptr] << "\n";
         map_rd_si_to_numsi[trans_ptr] = numsi;                //Maps si_trans to corresponding SI port
         std::cout << "Filled: map_rd_si_to_numsi[" << trans_ptr << "]: " << map_rd_si_to_numsi[trans_ptr] << "\n";
      }
   }
   ...
   for (int num_trans = 0; num_trans < num_mi_transaction; num_trans++) {
      ...
      std::cout << "Round: " << num_trans << "\n";
      std::cout << std::dec << "s_data_size: " << s_data_size << "\n";
      std::cout << std::dec << "m_data_size: " << m_data_size << "\n";
      std::cout << std::dec << "m_num_burst: " << m_num_burst << "\n";
      ...
      std::cout << std::dec << "burst_cnt: " << burst_cnt << "\n";
      std::cout << std::dec << "num_burst: " << num_burst << "\n";
      std::cout << std::dec << "fill_null_bytes: " << fill_null_bytes << "\n";
         ...
         for (int i = 0; i < burst_cnt; i++) {
            for (int j = 0; j < num_burst; j++) {
               if (s_data_size_abs > 0) {
                  ...
                  std::cout << "Wrote data_ptr[" << i * num_burst + j << "] = s_data_ptr[" << s_data_ptr_indx + (i * num_burst) + j << "] : " << std::hex << static_cast<int>(s_data_ptr[s_data_ptr_indx + (i * num_burst) + j]) << "\n";
               } else {
                  data_ptr[i * num_burst + j]    = 0x0;
                  data_en_ptr[i * num_burst + j] = 0x0;
                  std::cout << "Wrote data_ptr[" << i * num_burst + j << "] = 0x0\n";
               }
               s_data_size_abs--;
               s_data_size--;
            }
...
void smartconnect_xtlm::process_saxi_wr_req() {
   ...
   for (int num_si = 0; num_si < smc_num_si; num_si++) {
      ...
      std::cout << "Checking for saxi_wr_util[" << num_si << "]->is_trans_available(): " <<  saxi_wr_util[num_si]->is_trans_available()
      ...
      if (saxi_wr_util[num_si]->is_trans_available() && saxi_wr_req_vec[num_si].empty()) {
         ...
         std::cout << "Checking for saxi_wr_resp_vec[" << num_si << "].empty(): " << saxi_wr_resp_vec[num_si].empty() << " \n"; 
...
//         trans_ptr->acquire();
         ...
         std::cout << "saxi_wr_resp_vec[" << num_si << "] size: " << saxi_wr_resp_vec[num_si].size() << "\n";

         std::cout << "BEGIN Incoming Write Paylod information on num_si " << num_si << "\n";
         trans_ptr->get_log(payload_msg, 3);
         std::cout << payload_msg << "\n";
         payload_msg = "";
         std::cout << "END Incoming Write Payload information on num_si " << num_si << "\n";
...
void smartconnect_xtlm::process_maxi_wr_req() {
   ...
   if (slave_wr_req >= 0) {
      if (maxi_wr_util[slave_wr_req]->is_slave_ready() && !maxi_wr_req_vec[slave_wr_req].empty()) {
         ...
         std::cout << "BEGIN Sent out Write Paylod information from num_mi " << slave_wr_req << "\n";
         trans_ptr->get_log(payload_msg, 3);
         std::cout << payload_msg << "\n";
         payload_msg = "";
         std::cout << "END Sent out Write Payload information from num_mi " << slave_wr_req << "\n";
         ...
         if (maxi_wr_req_vec[slave_wr_req].empty()) {
            std::cout << "process_maxi_wr_req: maxi_wr_req_vec[" << slave_wr_req << "].empty()-> empty!!!\n";
         } else {
            std::cout << "process_maxi_wr_req: maxi_wr_util[" << slave_wr_req << "]->is_slave_ready()-> not ready!!!\n";
         }
      }
   } else {
      std::cout << "slave_wr_req is: " << slave_wr_req << "\n";
...
void smartconnect_xtlm::process_maxi_wr_resp() {
   ...
   for (int num_mi = 0; num_mi < smc_num_mi; num_mi++) {
      if (maxi_wr_util[num_mi]->is_resp_available()) {
         ...
         std::cout << "Accessed: in process_maxi_wr_resp() si_trans: " << si_trans << "for mi_trans: " << mi_trans << "\n";
...
//         maxi_wr_resp_vec[num_mi].push_back(mi_trans);
         ...
         std::cout << "Accessed: in process_maxi_wr_resp() map_wr_si_to_nummi[" << si_trans << "] before: " << map_wr_si_to_nummi[si_trans] << "\n";
         map_wr_si_to_nummi[si_trans]--;
         std::cout << "Accessed: in process_maxi_wr_resp() map_wr_si_to_nummi[" << si_trans << "] after: " << map_wr_si_to_nummi[si_trans] << "\n";
         if (map_wr_si_to_nummi[si_trans] == 0) {
            slave_wr_resp.push_back(si_trans);
            slave_wr_resp_nummi.push_back(num_mi);
            std::cout << "Accessed: in process_maxi_wr_resp() notifying send_wr_resp with si_trans: " << slave_wr_resp.back() << "\n";
...
//   xtlm::aximm_payload* trans_ptr = saxi_wr_resp_vec[num_si].front();
   ...
   while(!slave_wr_resp.empty()) {
      ...
      std::cout << "Processing process_saxi_wr_resp on num_si: " << num_si << "\n";
      std::cout << "BEGIN Response information\n";
      trans_ptr->get_log(payload_msg, 3);
      std::cout << payload_msg << "\n";
      payload_msg = "";
      std::cout << "RESPONSE: " << trans_ptr->get_response_string() << "\n";
      std::cout << "END Response information\n\n";
   
      if (saxi_wr_util[num_si]->is_master_ready() == false) {
         std::cout << "Processing process_saxi_wr_resp on num_si: " << num_si << " master not ready\n";
      ...
      if (trans_ptr != NULL) {
         std::cout << "BEGIN sending Write Response information on num_si " << num_si << "\n";
         trans_ptr->get_log(payload_msg, 3);
         std::cout << payload_msg << "\n";
         payload_msg = "";
         std::cout << "RESPONSE: " << trans_ptr->get_response_string() << "\n";
         std::cout << "END sending Write Response information on num_si " << num_si << "\n";
   
         print_log("SI", num_si, "WRITE", "Response sent", trans_ptr);
   
         saxi_wr_util[num_si]->send_resp(*trans_ptr, delay);
         std::cout << "INFO: " << std::string(this->name()) << ": process_saxi_wr_resp(): SAXI WR RESP sent \n\n";
         map_wr_si_to_nummi.erase(trans_ptr);
         map_wr_si_to_numsi.erase(trans_ptr);
   //      trans_ptr->release();
         create_wr_resp = false;
         bad_saxi_wr_trans = -1;
         std::cout << "INFO: " << std::string(this->name()) << ": process_saxi_wr_resp(): SAXI WR RESP released and sent \n\n";
      }
...
void smartconnect_xtlm::process_saxi_rd_req() {
   ...
   for (int num_si = 0; num_si < smc_num_si; num_si++) {
      ...
      std::cout << "Checking for saxi_rd_util[" << num_si << "]->is_trans_available(): " <<  saxi_rd_util[num_si]->is_trans_available()
      ...
      if (saxi_rd_util[num_si]->is_trans_available() && saxi_rd_req_vec[num_si].empty()) {
         ...
         std::cout << "Checking for saxi_rd_resp_vec[" << num_si << "].empty(): " << saxi_rd_resp_vec[num_si].empty() << " \n";
...
//         trans_ptr->acquire();
         ...
         std::cout << "saxi_rd_resp_vec[" << num_si << "] size: " << saxi_rd_resp_vec[num_si].size() << "\n";

         print_log("SI", num_si, "READ", "Transaction received", trans_ptr);

         std::cout << "BEGIN Incoming Read Paylod information on num_si " << num_si << "\n";
         trans_ptr->get_log(payload_msg, 3);
         std::cout << payload_msg << "\n";
         payload_msg = "";
         std::cout << "END Incoming Read Payload information on num_si " << num_si << "\n";
...
void smartconnect_xtlm::process_maxi_rd_req() {
   ...
   if (slave_rd_req >= 0) {
      if (maxi_rd_util[slave_rd_req]->is_slave_ready() && !maxi_rd_req_vec[slave_rd_req].empty() ) {
         ...
         std::cout << "BEGIN process_maxi_rd_req() sending READ payload\n";
         trans_ptr->get_log(payload_msg, 3);
         std::cout << payload_msg << "\n";
         payload_msg = "";
         std::cout << "END process_maxi_rd_req() sending READ payload\n\n";
         ...
         if (!maxi_rd_util[slave_rd_req]->is_slave_ready()) {
            std::cout << "process_maxi_rd_req: maxi_rd_util[" << slave_rd_req << "]->is_slave_ready()-> not ready!!!\n";
         } else {
            std::cout << "process_maxi_rd_req: maxi_rd_req_vec[" << slave_rd_req << "] reached maxi_rd_req_vec[" << slave_rd_req << "].end()\n";
         }
      }
   } else {
      std::cout << "slave_rd_req is: " << slave_rd_req << "\n";
...
void smartconnect_xtlm::process_maxi_rd_resp() {
   ...
   for (int num_mi = 0; num_mi < smc_num_mi; num_mi++) {
      if (maxi_rd_util[num_mi]->is_data_available()) {
         ...
         std::cout << "BEGIN Regslice receiving READ information\n";
         mi_trans->get_log(payload_msg, 3);
         std::cout << payload_msg << "\n";
         payload_msg = "";
         std::cout << "END Regslice receiving READ information\n\n";
...
void smartconnect_xtlm::process_saxi_rd_resp() {
   ...
   while (!slave_rd_resp.empty()) {
      ...
      std::cout << "num_si is: " << num_si << "\n";
      std::cout << "SI trans is is: " << trans_ptr << "\n";
      ...
      std::cout << "Processing process_saxi_rd_resp on num_si: " << num_si << "\n";
      std::cout << "BEGIN Response information\n";
      trans_ptr->get_log(payload_msg, 3);
      std::cout << payload_msg << "\n";
      payload_msg = "";
      ...
      if (saxi_rd_util[num_si]->is_master_ready() == false) {
         std::cout << "Processing process_saxi_rd_resp on num_si: " << num_si << " master not ready\n";
      ...
      //   int s_data_size = s_burst_cnt * s_num_burst;
         ...
         std::cout << "saxi burst_length is: " << s_burst_length << "\n";
         std::cout << "saxi burst_size is: " << s_burst_size << "\n";
         std::cout << "saxi start addr is: " << std::hex << s_addr << "\n";
         std::cout << "saxi end addr is: " << std::hex << s_data_ptr_indx_high << "\n";
         std::cout << "saxi data_ptr is: " << std::hex << s_data_ptr << "\n";
         ...
         while (mmap_rd_si_to_mi_itr != mmap_rd_si_to_mi.end()) {
            ...
            std::cout << "maxi transaction is: " << m_req_trans_ptr << "\n";
            ...
            if (!mi_cascaded) { //MI not cascaded so these are orinigal trans responses  so extract read data here
               ...
               std::cout << "BEGIN Regslice vector pop out READ information\n";
               m_resp_trans_ptr->get_log(payload_msg, 3);
               std::cout << payload_msg << "\n";
               payload_msg = "";
               std::cout << "END Regslice vector pop out READ information\n\n";
         ...
         //         break;
                  ...
                  for (int i = 0; i < m_burst_cnt; i++) {
                     for (int j = 0; j < m_num_burst; j++) {
                        ...
                        std::cout << "Read s_data_ptr[" << s_data_ptr_indx << "] = m_data_ptr[" << (i*m_num_burst) + j << "]: "<< std::hex << static_cast<int>(m_data_ptr[(i*m_num_burst) + j]) << "\n";
                        s_data_ptr_indx++;
      ...
      if (trans_ptr != NULL) {
         std::cout << "BEGIN sending Read Response information on num_si " << num_si << "\n";
         trans_ptr->get_log(payload_msg, 3);
         std::cout << payload_msg << "\n";
         payload_msg = "";
         std::cout << "RESPONSE: " << trans_ptr->get_response_string() << "\n";
         std::cout << "END sending Read Response information on num_si " << num_si << "\n\n";
